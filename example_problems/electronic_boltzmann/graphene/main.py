...
print("mu = ", af.mean(params.mu[N_g:-N_g, N_g:-N_g]),
      ...
      "density = ", af.mean(n_nls[N_g:-N_g, N_g:-N_g])
     )

for time_index, t0 in enumerate(time_array):
    print("Time step =", time_index, ", Time =", t0)

    N_g = domain.N_ghost
    mean_density = af.mean(n_nls[N_g:-N_g, N_g:-N_g])
    density_pert = n_nls - mean_density

    if (time_index%1==0):
        pl.contourf(np.array(nls.q1_center)[N_g:-N_g, N_g:-N_g], \
                    np.array(nls.q2_center)[N_g:-N_g, N_g:-N_g], \
                    np.array(density_pert)[N_g:-N_g, N_g:-N_g], \
                    100, cmap='bwr'
                   )
#        pl.contourf(np.array(nls.q1_center)[N_g:-N_g, N_g:-N_g], \
#                    np.array(nls.q2_center)[N_g:-N_g, N_g:-N_g], \
#                    np.log10(np.array(n_nls))[N_g:-N_g, N_g:-N_g], \
#                    100, cmap='hot'
#                   )
        pl.title('Time = ' + "%.2f"%(t0) )
        pl.xlabel('$x$')
