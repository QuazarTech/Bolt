...
def plot_1d():
    for time_index, t0 in enumerate(time_array):
        ...
        ax1.plot(q2[0, :], np.mean(n[:, :, 0], 0), color = 'C0', label = 'Electrons')
        ax1.plot(q2[0, :], np.mean(n[:, :, 1], 0), '--', color = 'C3', label = 'Ions')
        ax1.legend()
        ...
        ax2.plot(q2[0, :], np.mean(v1[:, :, 0] / params.v0, 0), color = 'C0')
        ax2.plot(q2[0, :], np.mean(v1[:, :, 1] / params.v0, 0), '--', color = 'C3')
        ax2.set_xlabel(r'$y(l_s)$')
        ax2.set_ylabel(r'$<v_x>(v_0)$')
        ax2.set_ylim([1.05 * v1_min / params.v0, 1.05 * v1_max / params.v0])

        ax3 = fig.add_subplot(2, 2, 3)
        ax3.plot(q2[0, :], np.mean(T[:, :, 0] / params.T0, 0), color = 'C0')
        ax3.plot(q2[0, :], np.mean(T[:, :, 1] / params.T0, 0), '--', color = 'C3')
        ax3.set_xlabel(r'$y(l_s)$')
        ax3.set_ylabel(r'$<T>(T_0)$')
        ax3.set_ylim([0.95 * T_min / params.T0, 1.05 * T_max / params.T0])

        ax4 = fig.add_subplot(2, 2, 4)
        ax4.plot(q2[0, :], np.mean(B1[:, :] / params.B0, 0))
