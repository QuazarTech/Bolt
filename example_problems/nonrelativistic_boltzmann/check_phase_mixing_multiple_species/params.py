...
# Setting plasma β:
beta = np.inf
B0   = np.sqrt(n0 * T0 / (2 * beta)) # |B| units(B)
...
temperature_background_1 = 1    * T0
temperature_background_2 = 0.1  * T0
temperature_background_3 = 0.01 * T0

# Velocity, length and time scales:
v0 = velocity_scales.sound_speed(temperature_background_1, k0, gamma)
l0 = 1 # Box Length
t0 = l0 / v0
...
# Velocity Scales:
thermal_speed   = velocity_scales.thermal_speed(temperature_background_1, m0, k0)
sound_speed     = velocity_scales.sound_speed(temperature_background_1, k0, gamma)
alfven_velocity = velocity_scales.alfven_velocity(B0, density_background, m0, mu) 

# Length scales:
debye_length = length_scales.debye_length(density_background, temperature_background_1, e0, k0, eps)
skin_depth   = length_scales.skin_depth(density_background, e0, c, m0, eps)
gyroradius   = length_scales.gyroradius(velocity_scales.thermal_speed(temperature_background_1, m0, k0), B0, e0, m0)
...
sound_crossing_time  = time_scales.sound_crossing_time(min(L_x, L_y), temperature_background_1, k0, gamma)
...
PETSc.Sys.Print("Maximum Velocity(1)  :", v_max_1 / v0, "|v0| units(v0)")
PETSc.Sys.Print("Maximum Velocity(2)  :", v_max_2 / v0, "|v0| units(v0)")
PETSc.Sys.Print("Maximum Velocity(3)  :", v_max_3 / v0, "|v0| units(v0)")
PETSc.Sys.Print("==================================================\n")
