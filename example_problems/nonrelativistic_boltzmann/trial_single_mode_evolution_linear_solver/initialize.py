...
def initialize_f(q1, q2, p1, p2, p3, params):
    ...
    rho = rho_b + (  pert_real * np.cos(k_q1 * q1 + k_q2 * q2)
                   - pert_imag * np.sin(k_q1 * q1 + k_q2 * q2)
                  )
    ...
    if (params.p_dim == 3):
        ...
        f = rho * (m / (2 * np.pi * k * T_b))**(3 / 2) \
                * np.exp(-m * (p1 - p1_bulk)**2 / (2 * k * T_b)) \
                * np.exp(-m * (p2 - p2_bulk)**2 / (2 * k * T_b)) \
                * np.exp(-m * (p3 - p3_bulk)**2 / (2 * k * T_b))
