...
def lowpass_filter(f):
    ...
    # Applying the filter:
    f_hat_filtered = 0.5 * (f_hat * (  af.tanh((k_v + 0.8 * af.max(k_v)) / 0.05)
                                     - af.tanh((k_v + 0.8 * af.min(k_v)) / 0.05)
                                    )
                           )

    # f_hat = af.select(af.abs(k_v) < 0.8 * af.max(k_v), f_hat, f_hat_filtered)
    # f = af.real(af.ifft(f_hat))
    f = af.real(af.ifft(f_hat_filtered))
    return(f) 
...
E_data_nls2   = np.zeros(time_array.size)
...
E_data_nls2[0]   = af.max(nls2.fields_solver.cell_centered_EM_fields[:, :, N_g:-N_g, N_g:-N_g])
...
for time_index, t0 in enumerate(time_array[1:]):
    ...
    E_data_nls[time_index+1]   = af.max(nls.fields_solver.cell_centered_EM_fields[:, :, N_g:-N_g, N_g:-N_g])

    n_nls2                        = nls2.compute_moments('density')
    rho_data_nls2[time_index + 1] = af.max(n_nls2[:, :, N_g:-N_g, N_g:-N_g]) 
    E_data_nls2[time_index+1]     = af.max(nls2.fields_solver.cell_centered_EM_fields[:, :, N_g:-N_g, N_g:-N_g])
...
h5f.create_dataset('E_nls2', data = E_data_nls2)
