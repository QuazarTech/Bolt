...
# (delta_u2_e, ' = ', -1.009663273127126e-17 + 0.14260527919242938*I)
# (delta_u3_e, ' = ', 0.1426052791924314 - 1.0535056984770086e-16*I)
# (delta_u2_i, ' = ', -1.4858990773913838e-16 + 0.6008293261570782*I)
# (delta_u3_i, ' = ', 0.6008293261570793)
# (delta_B2, ' = ', 3.1143707404646115e-17 - 0.3085267277273554*I)
# (delta_B3, ' = ', -0.308526727727356 - 5.434563389583591e-17*I)
# (delta_E2, ' = ', -0.15322895762201727 + 6.667843360785852e-18*I)
# (delta_E3, ' = ', -1.421524573400057e-16 + 0.15322895762201585*I)
...
def initialize_f(q1, q2, v1, v2, v3, params):
    ...
    v2_bulk_i =   params.amplitude * -5.745404152435185e-15 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 0.6008293261570782 * af.sin(params.k_q1 * q1)

    v2_bulk_e =   params.amplitude * -5.88418203051333e-15 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 0.14260527919242938 * af.sin(params.k_q1 * q1)
    
    v3_bulk_i =   params.amplitude * 0.6008293261570793 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 2.220446049250313e-16  * af.sin(params.k_q1 * q1)

    v3_bulk_e =   params.amplitude * 0.1426052791924314 * af.cos(params.k_q1 * q1) \
...
def initialize_B(q1, q2, params):
    ...
    omega = -8.164138624051161e-17 - 0.7449709077916147 * 1j

    B2 = (params.amplitude * (3.1143707404646115e-17 - 0.3085267277273554*1j) * \
          np.exp(  1j * params.k_q1 * np.array(q1)
                 + omega * dt / 2
                )).real

    B3 = (params.amplitude * (-0.308526727727356 - 5.434563389583591e-17*1j) * \
          np.exp(  1j * params.k_q1 * np.array(q1)
                 + omega * dt / 2
                )).real
