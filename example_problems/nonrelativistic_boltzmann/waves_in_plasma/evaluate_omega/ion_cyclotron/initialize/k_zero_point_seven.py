...
# (delta_u2_e, ' = ', 3.608224830031759e-16 + 0.24199032917949193*I)
# (delta_u3_e, ' = ', 0.24199032917949223 + 1.942890293094024e-16*I)
# (delta_u2_i, ' = ', 1.3877787807814457e-16 + 0.4931610686541087*I)
# (delta_u3_i, ' = ', 0.4931610686541088)
# (delta_B2, ' = ', -5.551115123125783e-17 - 0.3658545632570288*I)
# (delta_B3, ' = ', -0.3658545632570295)
# (delta_E2, ' = ', -0.2537386046535821)
# (delta_E3, ' = ', 2.636779683484747e-16 + 0.25373860465358244*I)
...
def initialize_f(q1, q2, v1, v2, v3, params):
    ...
    v2_bulk_i =   params.amplitude * -5.745404152435185e-15 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 0.4931610686541087 * af.sin(params.k_q1 * q1)

    v2_bulk_e =   params.amplitude * -5.88418203051333e-15 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 0.24199032917949193 * af.sin(params.k_q1 * q1)
    
    v3_bulk_i =   params.amplitude * 0.4931610686541088 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 2.220446049250313e-16  * af.sin(params.k_q1 * q1)

    v3_bulk_e =   params.amplitude * 0.24199032917949223 * af.cos(params.k_q1 * q1) \
...
def initialize_B(q1, q2, params):
    ...
    omega = -1.0386656874380749e-16 - 0.4854853296792793 * 1j

    B2 = (params.amplitude * (-5.551115123125783e-17 - 0.3658545632570288*1j) * \
          np.exp(  1j * params.k_q1 * np.array(q1)
                 + omega * dt / 2
                )).real

    B3 = (params.amplitude * (-0.3658545632570295 + 6.821156324238881e-17*1j) * \
          np.exp(  1j * params.k_q1 * np.array(q1)
                 + omega * dt / 2
                )).real
