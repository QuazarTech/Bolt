...
# (delta_u2_e, ' = ', 1.5147040209495424e-17 + 0.06718286752328949*I)
# (delta_u3_e, ' = ', 0.06718286752329118 - 2.88987789268466e-17*I)
# (delta_u2_i, ' = ', -1.0527348984950072e-17 + 0.663966816083932*I)
# (delta_u3_i, ' = ', 0.6639668160839327)
# (delta_B2, ' = ', -9.55347285384444e-18 - 0.22199552855713947*I)
# (delta_B3, ' = ', -0.22199552855713992 + 4.1344101671956027e-17*I)
# (delta_E2, ' = ', -0.07316088265835553 - 5.0555161456872916e-18*I)
# (delta_E3, ' = ', 1.3691546438536917e-17 + 0.07316088265835408*I)
...
def initialize_f(q1, q2, v1, v2, v3, params):
    ...
    v2_bulk_i =   params.amplitude * -5.745404152435185e-15 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 0.663966816083932 * af.sin(params.k_q1 * q1)

    v2_bulk_e =   params.amplitude * -5.88418203051333e-15 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 0.06718286752328949 * af.sin(params.k_q1 * q1)
    
    v3_bulk_i =   params.amplitude * 0.6639668160839327 * af.cos(params.k_q1 * q1) \
                - params.amplitude * 2.220446049250313e-16  * af.sin(params.k_q1 * q1)

    v3_bulk_e =   params.amplitude * 0.06718286752329118 * af.cos(params.k_q1 * q1) \
...
def initialize_B(q1, q2, params):
    ...
    omega = -8.164138624051161e-17 - 0.8898124410947876 * 1j

    B2 = (params.amplitude * (3.1143707404646115e-17 - 0.22199552855713947*1j) * \
          np.exp(  1j * params.k_q1 * np.array(q1)
                 + omega * dt / 2
                )).real

    B3 = (params.amplitude * (-0.22199552855713992 - 5.434563389583591e-17*1j) * \
          np.exp(  1j * params.k_q1 * np.array(q1)
                 + omega * dt / 2
                )).real
