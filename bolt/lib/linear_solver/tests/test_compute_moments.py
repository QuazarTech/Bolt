...
class test(object):
    def __init__(self):
        ...
        self.p1 = af.reorder(af.flat(af.to_array(self.p1)),
                             2, 3, 0, 1)

        self.p2 = af.reorder(af.flat(af.to_array(self.p2)),
                             2, 3, 0, 1)

        self.p3 = af.reorder(af.flat(af.to_array(self.p3)),
                             2, 3, 0, 1)
...
def test_compute_moments():
    ...
    error_rho = af.sum(af.abs(rho_calc - rho_ana))/rho_calc.elements()
    ...
    error_E = af.sum(af.abs(E_calc - E_ana))/E_calc.elements()
    
    mom_p1b_calc = compute_moments(obj, 'mom_p1_bulk')
    mom_p1b_ana  = (1 + 0.01 * af.sin(2 * np.pi * obj.q1 + 4 * np.pi * obj.q2)) *\
                   (0.01 * af.exp(-10 * obj.q1**2 - 10 * obj.q2**2))

    error_p1b = af.sum(af.abs(mom_p1b_calc - mom_p1b_ana))/mom_p1b_calc.elements()

    mom_p2b_calc = compute_moments(obj, 'mom_p2_bulk')
    mom_p2b_ana  = (1 + 0.01 * af.sin(2 * np.pi * obj.q1 + 4 * np.pi * obj.q2)) *\
                   (0.01 * af.exp(-10 * obj.q1**2 - 10 * obj.q2**2))

    error_p2b = af.sum(af.abs(mom_p2b_calc - mom_p2b_ana))/mom_p2b_calc.elements()

    mom_p3b_calc = compute_moments(obj, 'mom_p3_bulk')
    mom_p3b_ana  = (1 + 0.01 * af.sin(2 * np.pi * obj.q1 + 4 * np.pi * obj.q2)) *\
                   (0.01 * af.exp(-10 * obj.q1**2 - 10 * obj.q2**2))

    error_p3b = af.sum(af.abs(mom_p3b_calc - mom_p3b_ana))/mom_p3b_calc.elements()
