...
velocities to be a sinusoidal function which is then checked
against the values returned by the compute_moments function
"""
...
class test(object):
    def __init__(self):
        ...
        self.p1 = af.tile(af.reorder(af.flat(af.to_array(self.p1)),
                                     2, 3, 0, 1),
                          self.N_q1 + 2 * self.N_ghost,
                          self.N_q2 + 2 * self.N_ghost,
                          1)

        self.p2 = af.tile(af.reorder(af.flat(af.to_array(self.p2)),
                                     2, 3, 0, 1),
                          self.N_q1 + 2 * self.N_ghost,
                          self.N_q2 + 2 * self.N_ghost,
                          1)

        self.p3 = af.tile(af.reorder(af.flat(af.to_array(self.p3)),
                          2, 3, 0, 1),
                          self.N_q1 + 2 * self.N_ghost,
                          self.N_q2 + 2 * self.N_ghost,
                          1)
        ...
        self.q1 = af.tile(af.to_array(self.q1),
                          1, 1, self.N_p1 * self.N_p2 * self.N_p3)
        self.q2 = af.tile(af.to_array(self.q2),
                          1, 1, self.N_p1 * self.N_p2 * self.N_p3)
...
def test_compute_moments():
    ...
    error_rho = af.sum(af.abs(rho_calc - (1 + 0.01 *
                                          af.sin(2 * np.pi * obj.q1[:, :, 0] +
                                                 4 * np.pi * obj.q2[:, :, 0]
                                                 )
                                          ))) / rho_calc.elements()
    ...
    error_E = af.sum(af.abs(E_calc - 3 * (1 + 0.01 *
                                          af.sin(2 * np.pi * obj.q1[:, :, 0] +
                                                 4 * np.pi * obj.q2[:, :, 0]
                                                 )
                                          )**2 -
    ...
    # This seems buggy ATM:
    # NEED TO FIX:
    error_p1 = af.sum(af.abs(mom_p1_calc -
                             (1 + 0.01 * af.sin(2 * np.pi * obj.q1[:, :, 0] +
                                                4 * np.pi * obj.q2[:, :, 0]
                                                )
                              ))**2) / rho_calc.elements()

    error_p2 = af.sum(af.abs(mom_p2_calc -
                             (1 + 0.01 * af.sin(2 * np.pi * obj.q1[:, :, 0] +
                                                4 * np.pi * obj.q2[:, :, 0]
                                                )
                              ))**2) / rho_calc.elements()

    error_p3 = af.sum(af.abs(mom_p3_calc -
                             (1 + 0.01 * af.sin(2 * np.pi * obj.q1[:, :, 0] +
                                                4 * np.pi * obj.q2[:, :, 0]
                                                )
                              ))**2) / rho_calc.elements()
