    ...
    def __init__(self,
        ...
        for i in range(len(attributes)):
            if(not (isinstance(getattr(boundary_conditions, attributes[i]), str) 
               ...
               or   isinstance(getattr(boundary_conditions, attributes[i]), float))
              ):
        ...
        if(type(params.charge) == af.array.Array):
            for i in range(N_species):
                PETSc.Sys.Print('   Charge(Species %1d)               :'%(i+1), 
                                af.sum(params.charge[0, i])
                               )
                PETSc.Sys.Print('   Mass(Species %1d)                 :'%(i+1), 
                                af.sum(params.mass[0, i])
                               )
        else:
            for i in range(N_species):
