...
def apply_bcs_f(self):
    ...
    if(self.physical_system.boundary_conditions.in_q2 == 'dirichlet'):
        ...
        if(i_q2_start == 0):
            ...
            # For mirror boundary conditions:
            # 0 = 5; 1 = 4; 2 = 3;
            self.f[:, :, :N_ghost] = af.flip(self.f[:, :, N_ghost:2 * N_ghost], 2)
        ...
        if(i_q2_end == self.N_q2 - 1):
            ...
            # For mirror boundary conditions:
            # -1 = -6; -2 = -5; -3 = -4;
            self.f[:, :, -N_ghost:] = af.flip(self.f[:, :, -2 * N_ghost:-N_ghost], 2)
...
def apply_bcs_fields(self):
    ...
    if(self.physical_system.boundary_conditions.in_q2 == 'dirichlet'):
        ...
        if(i_q2_start == 0):
            ...
            self.E1[:, :, :N_ghost] = af.flip(self.E1[:, :, N_ghost:2 * N_ghost], 2)
            self.E2[:, :, :N_ghost] = af.flip(self.E2[:, :, N_ghost:2 * N_ghost], 2)
            self.E3[:, :, :N_ghost] = af.flip(self.E3[:, :, N_ghost:2 * N_ghost], 2)
            
            self.B1[:, :, :N_ghost] = af.flip(self.B1[:, :, N_ghost:2 * N_ghost], 2)
            self.B2[:, :, :N_ghost] = af.flip(self.B2[:, :, N_ghost:2 * N_ghost], 2)
            self.B3[:, :, :N_ghost] = af.flip(self.B3[:, :, N_ghost:2 * N_ghost], 2)
        ...
        if(i_q2_end == self.N_q2 - 1):
            ...
            self.E1[:, :, -N_ghost:] = af.flip(self.E1[:, :, -2 * N_ghost:-N_ghost], 2)
            self.E2[:, :, -N_ghost:] = af.flip(self.E2[:, :, -2 * N_ghost:-N_ghost], 2)
            self.E3[:, :, -N_ghost:] = af.flip(self.E3[:, :, -2 * N_ghost:-N_ghost], 2)

            self.B1[:, :, -N_ghost:] = af.flip(self.B1[:, :, -2 * N_ghost:-N_ghost], 2)
            self.B2[:, :, -N_ghost:] = af.flip(self.B2[:, :, -2 * N_ghost:-N_ghost], 2)
            self.B3[:, :, -N_ghost:] = af.flip(self.B3[:, :, -2 * N_ghost:-N_ghost], 2)
