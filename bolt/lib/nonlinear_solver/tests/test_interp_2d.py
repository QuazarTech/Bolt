...
class test(object):
    def __init__(self, N_q1, N_q2, N_ghost):
        ...
        self.q1_center = af.to_array(
            (-N_ghost + np.arange(N_q1 + 2 * N_ghost) + 0.5) * (1 / N_q1))
        self.q2_center = af.to_array(
            (-N_ghost + np.arange(N_q2 + 2 * N_ghost) + 0.5) * (1 / N_q2))
        self.q1_center = af.tile(self.q1_center, 1, N_q2 + 2 * N_ghost)
        self.q2_center = af.tile(
            af.reorder(self.q2_center), N_q1 + 2 * N_ghost, 1)
