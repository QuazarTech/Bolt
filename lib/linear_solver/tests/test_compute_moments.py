...
class test(object):
  def __init__(self):
    ...
    self.p1 = af.tile(af.reorder(af.flat(af.to_array(self.p1)), 2, 3, 0, 1),\
                      self.N_q1 + 2*self.N_ghost, self.N_q2 + 2*self.N_ghost, 1
                     )
    
    self.p2 = af.tile(af.reorder(af.flat(af.to_array(self.p2)), 2, 3, 0, 1),\
                      self.N_q1 + 2*self.N_ghost, self.N_q2 + 2*self.N_ghost, 1
                     )

    self.p3 = af.tile(af.reorder(af.flat(af.to_array(self.p3)), 2, 3, 0, 1),\
                      self.N_q1 + 2*self.N_ghost, self.N_q2 + 2*self.N_ghost, 1
                     )

    self.q1 = (-self.N_ghost + 0.5 + np.arange(self.N_q1+2*self.N_ghost))/self.N_q1
    self.q2 = (-self.N_ghost + 0.5 + np.arange(self.N_q2+2*self.N_ghost))/self.N_q2

    self.q2, self.q1 = np.meshgrid(self.q2, self.q1)

    self.q1 = af.tile(af.to_array(self.q1), 1, 1, self.N_p1 * self.N_p2 * self.N_p3)
