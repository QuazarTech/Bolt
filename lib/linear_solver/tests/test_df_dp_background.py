...
def test_df_dp_background():
  ...
  for i in range(N.size):
    ...
    dfdp3_expected = -2 * obj.p3 * af.exp(-obj.p1**2)*af.exp(-obj.p2**2)*af.exp(-obj.p3**2)

    dfdp1_expected = af.tile(af.reorder(af.flat(dfdp1_expected), 2, 3, 0, 1), obj.N_q1, obj.N_q2, 1)
    dfdp2_expected = af.tile(af.reorder(af.flat(dfdp2_expected), 2, 3, 0, 1), obj.N_q1, obj.N_q2, 1)
    dfdp3_expected = af.tile(af.reorder(af.flat(dfdp3_expected), 2, 3, 0, 1), obj.N_q1, obj.N_q2, 1)

    error_1[i] = af.sum(af.abs(dfdp1_expected - obj.dfdp1_background))/dfdp1_expected.elements()
    error_2[i] = af.sum(af.abs(dfdp2_expected - obj.dfdp2_background))/dfdp2_expected.elements()
    error_3[i] = af.sum(af.abs(dfdp3_expected - obj.dfdp3_background))/dfdp3_expected.elements()
  ...
  assert(abs(poly_1[0] + 4)<0.2 and abs(poly_2[0] + 4)<0.2 and abs(poly_3[0] + 4)<0.2)