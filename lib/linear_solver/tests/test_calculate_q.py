...
def test_calculate_q_center():
  ...
  q1_expected = test_obj.q1_start + (0.5 + np.arange(test_obj.N_q1)) * test_obj.dq1
  q2_expected = test_obj.q2_start + (0.5 + np.arange(test_obj.N_q2)) * test_obj.dq2

  q1_expected = af.tile(af.to_array(q1_expected), 1, test_obj.N_q2, test_obj.N_p1 * test_obj.N_p2 * test_obj.N_p3)
  q2_expected = af.tile(af.reorder(af.to_array(q2_expected)), test_obj.N_q1, 1, test_obj.N_p1 * test_obj.N_p2 * test_obj.N_p3)

  assert(af.sum(af.abs(q1_expected - q1)) + af.sum(af.abs(q2_expected - q2))==0)