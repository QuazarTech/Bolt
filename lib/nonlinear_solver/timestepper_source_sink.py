...
def RK2_step(self, dt, testing=0):
    ...
    self.f = self.f + self._source_or_sink(*args) * (dt / 2)
...
def RK4_step(self, dt, testing=0):
    ...
    k1 = self._source_or_sink(*args)
    self.f = f_initial + 0.5 * k1 * dt
    k2 = self._source_or_sink(*args)
    self.f = f_initial + 0.5 * k2 * dt
    k3 = self._source_or_sink(*args)
    self.f = f_initial + k3 * dt
    k4 = self._source_or_sink(*args)
