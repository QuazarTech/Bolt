...
class nonlinear_solver(object):
  ...
  def __init__(self, physical_system):
    ...
    physical_system: The defined physical system object which holds all the simulation
                     information such as the initial conditions, and the domain info
                     is passed as an argument in defining an instance of the 
                     nonlinear_solver. This system is then evolved, and monitored using
                     the various methods under the nonlinear_solver class.
    """
    ...
    # This DA object is used in the communication routines for the EM field quantities.
    # A DOF of 6 is taken so that the communications, and application of B.C's may be
    # carried out in a single call among all the field quantities(E1, E2, E3, B1, B2, B3)
    ...
    self._A_q1 = physical_system.A_q(self.p1, self.p2, self.p3, physical_system.params)[0]
    self._A_q2 = physical_system.A_q(self.p1, self.p2, self.p3, physical_system.params)[1]
  ...
  def _initialize(self, params):
    ...
    self.E1 = af.constant(0, self.f.shape[0], self.f.shape[1], dtype = af.Dtype.f64)
    self.E2 = af.constant(0, self.f.shape[0], self.f.shape[1], dtype = af.Dtype.f64)
    self.E3 = af.constant(0, self.f.shape[0], self.f.shape[1], dtype = af.Dtype.f64)
    ...
    if(self.physical_system.params.fields_initialize == 'electrostatic'):
      ...
      self.E1, self.E2, self.E3 = self.physical_system.initial_conditions.\
                                  initialize_E(self.q1_center, self.q2_center,\
                                               params
                                              )

      self.B1, self.B2, self.B3 = self.physical_system.initial_conditions.\
