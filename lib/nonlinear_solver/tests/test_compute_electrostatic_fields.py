...
class test(object):
    def __init__(self, N):
        ...
        self._da_ksp = PETSc.DMDA().create(
            [self.N_q1, self.N_q2],
...
def test_compute_electrostatic_fields():
    ...
    for i in range(N.size):
        ...
        E1_expected = -(0.1 / np.pi) * af.cos(
            2 * np.pi * obj.q1 + 4 * np.pi * obj.q2)
        E2_expected = -(0.2 / np.pi) * af.cos(
            2 * np.pi * obj.q1 + 4 * np.pi * obj.q2)

        N_g = obj.N_ghost

        error_E1[i] = af.sum(
            af.abs(obj.E1[N_g:-N_g, N_g:-N_g] - E1_expected[N_g:-N_g, N_g:-N_g]
                   )) / (obj.E1[N_g:-N_g, N_g:-N_g].elements())

        error_E2[i] = af.sum(
            af.abs(obj.E2[N_g:-N_g, N_g:-N_g] - E2_expected[N_g:-N_g, N_g:-N_g]
