...
def test_calculate_p():
    ...
    p1_expected = af.tile(
        af.reorder(af.flat(af.to_array(p1_expected)), 2, 3, 0, 1),
        obj.N_q1 + 2 * obj.N_ghost, obj.N_q2 + 2 * obj.N_ghost, 1, 1)

    p2_expected = af.tile(
        af.reorder(af.flat(af.to_array(p2_expected)), 2, 3, 0, 1),
        obj.N_q1 + 2 * obj.N_ghost, obj.N_q2 + 2 * obj.N_ghost, 1, 1)

    p3_expected = af.tile(
        af.reorder(af.flat(af.to_array(p3_expected)), 2, 3, 0, 1),
        obj.N_q1 + 2 * obj.N_ghost, obj.N_q2 + 2 * obj.N_ghost, 1, 1)

    assert (
        af.sum(af.abs(p1_expected - p1)) + af.sum(af.abs(p2_expected - p2)) +
