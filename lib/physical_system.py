...
class physical_system(object):
  ...
  def __init__(self, domain, boundary_conditions, params,\
    ...
    params: This file contains details of the parameters that are to be used in the initialization 
            function. Additionally, it can also store the parameters that are to be used by other
            methods of the object.

    initial_conditions: Module containing functions which takes in the arrays as generated by domain, 
                        and assigns an initial value to the distribution function/Fields being evolved. 
                        It is also ensured that the I.C's are consistent with the B.C's
    ...
    if(isinstance(initial_conditions, types.ModuleType)==False):
