...
import linear_solver.evolve
import linear_solver.export
...
for i in range(len(config)):
  ...
  delta_f_hat_initial              = linear_solver.initialize.init_delta_f_hat(config[i])
  delta_rho_hat, delta_f_hat_final = linear_solver.evolve.time_integration(config[i], delta_f_hat_initial, time_array)

  f_lt = linear_solver.export.export_5D_distribution_function(config[i], delta_f_hat_final)
