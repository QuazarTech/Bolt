...
def ddelta_f_hat_dt(delta_f_hat, delta_E_x_hat, delta_E_y_hat, delta_B_z_hat, config):
  ...
  -----------
    ...
    delta_f_hat: Mode perturbation of the distribution function that is passed 
                 to the function. The array fed to this function is the result 
                 of the last time-step's integration.At t=0 the initial mode 
                 perturbation of the system as declared in the configuration
                 file is passed to this function.
  
  Output:
  -------
    ddelta_f_hat_dt : Array which contains the values of the derivative of the 
                      Fourier mode perturbation of the distribution function with 
                      respect to time.
  """
  ...
  delta_mom_bulk_x = np.sum(vel_x * delta_f_hat) * dv_x *dv_y
  delta_mom_bulk_y = np.sum(vel_y * delta_f_hat) * dv_x *dv_y

  dfdv_x_background = initialize.dfdv_x_background(config)

  dfdv_y_background = initialize.dfdv_y_background(config)

  delta_J_x_hat, delta_J_y_hat = charge_particle * delta_mom_bulk_x,\
                                 charge_particle * delta_mom_bulk_y
...
def time_integration(config, delta_f_hat_initial, time_array):
  ...
  -----------   
    ...
    delta_f_hat_initial : Array containing the initial values of the delta_f_hat. 
                          The value for this function is typically obtained from 
                          the appropriately named function from the initialize submodule.
    
    time_array : Array which consists of all the time points at which we are evolving 
                 the system. Data such as the mode amplitude of the density perturbation 
                 is also computed at the time points.
  Output:
  -------
    density_data : The value of the amplitude of the mode expansion of the density 
                   perturbation computed at the various points in time as declared in 
                   time_array

    new_delta_f_hat : This value that is returned by the function is the distribution 
                      function that is obtained at the final time-step. This is particularly 
                      useful in cases where comparisons need to be made between results of 
                      the Cheng-Knorr and the linear theory codes.
